...
#import "ZYFilterControl.h"



@implementation ZYFilterControl
...
-(id)initWithFrame:(CGRect)frame Interval:(CGFloat)interval titleItems:(NSArray *)titles LeftBackImage:(UIImage *)lImg MiddleBackImage:(UIImage *)mIMg Right:(UIImage *)rImg HLeftBackImage:(UIImage *)hlImg HMiddleBackImage:(UIImage *)hmIMg HRight:(UIImage *)hrImg {
    ...
    self.checkImages = [NSMutableArray array];
    self.lastIndex = 0;
    self.currentIndex = 0;
...
-(void)btnAction:(UIButton*)sender
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(ZYFilterControl:didSelectedAtIndex:)]) {
        [self.delegate ZYFilterControl:self didSelectedAtIndex:i];
    }
    if (self.block) {
