...
+ (int)getDismissQueenCount{
    return [dismissQueue count];
}

+ (int)getRetainQueenCount{
    return [retainQueue count];
}
...
//    [self performLayoutOfButtons];  //del by Jerry 11/3/14
//    [self centerViews];
...
+(void)dismissCurrentHUDWithSuperView:(UIView *)view{
    if(dismissQueue.count > 0)
    {
        MBAlertView *current = [dismissQueue lastObject];
        if ([current superView] == view || view == nil) {
...
+(void)dismissCurrentHUD
{
    if(dismissQueue.count > 0)
    {
        ...
        // delete by easy begin on 2014-10-28
        // dismiss方法中会删除当前的hud，此处多删除一个
//        [dismissQueue removeLastObject];
        // delete by easy end on 2014-10-28
    }
}
...
+ (void)removeAllRetainQueueObject{
    [retainQueue removeAllObjects];
}
...
-(void)centerViews
{
    [_buttons enumerateObjectsUsingBlock:^(MBAlertViewButton *button, NSUInteger idx, BOOL *stop)
    {
        if(_imageView)
        {
            [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : _imageView, @"offset" : [NSNumber numberWithFloat:0]}, @{@"view" : self.bodyLabelButton, @"offset" : [NSNumber numberWithFloat:20]}, @{@"view" : button, @"offset" : [NSNumber numberWithFloat:20]}]superView:self.superView];
        }
        else
        {
            [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : self.bodyLabelButton, @"offset" : [NSNumber numberWithFloat:0]}, @{@"view" : button, @"offset" : [NSNumber numberWithFloat:20]}] superView:self.superView];
        }
    }];
}
