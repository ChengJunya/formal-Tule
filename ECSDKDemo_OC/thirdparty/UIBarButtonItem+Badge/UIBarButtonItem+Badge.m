...
NSString const *redDotKey = @"redDotKey";
...
- (void)badgeInit
{
    ...
    self.badgeBGColor   = RGBColor(255.f, 80.f, 4.f, 1.f);
    self.badgeTextColor = [UIColor whiteColor];
    self.badgeFont      = [UIFont systemFontOfSize:12.0];
    self.badgePadding   = 6;
    self.badgeMinSize   = 6;//8;
    self.badgeOriginX   = self.customView.frame.size.width - self.badge.frame.size.width;
    self.badgeOriginY   = -6;//-4;
...
-(UIView*) redDot {
    return objc_getAssociatedObject(self, &redDotKey);
}
-(void)setRedDot:(UIView *)redDot
{
    objc_setAssociatedObject(self, &redDotKey, redDot, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(void) setRedDotVisable:(BOOL)isVisable
{
    if (!self.redDot) {
        UIView *redDot = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.customView.frame) - 8.f, self.badgeOriginY + 4.f, 8.f, 8.f)];
        redDot.backgroundColor = RGBColor(255.f, 80.f, 4.f, 1.f);
        redDot.layer.cornerRadius = 4.f;
        [self.customView addSubview:redDot];
        self.redDot = redDot;
    }
    self.redDot.hidden = (!isVisable || self.badge);
}
