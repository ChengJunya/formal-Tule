...
@interface ChatViewController()<UITableViewDataSource, UITableViewDelegate,HPGrowingTextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,MWPhotoBrowserDelegate,CustomEmojiViewDelegate>
{
    BOOL isGroup;
...
@property (nonatomic,strong) UserInfoDTO *logingUserDTO;
@end
...
{
    if (self = [super init]) {
//        NSString *aSessionId = [self.itemData valueForKey:@"SESSION_ID"];
...
-(void)viewDidLoad{
    ...
    NSString *chatBgImageName = [GUserDefault valueForKey:CHAT_BG_IMAGE_NAME];
    ...
    self.logingUserDTO = [GUserDataHelper tlUserInfo];
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0f, yOffSet, self.view.frame.size.width,self.view.frame.size.height-ToolbarInputViewHeight-NAVIGATIONBAR_HEIGHT-STATUSBAR_HEIGHT) style:UITableViewStylePlain];
    ...
    //self.title =[[DemoGlobalClass sharedInstance]getOtherNameWithVoip:self.sessionId];
    self.myVoip = [DemoGlobalClass sharedInstance].loginInfoDic[voipKey];
    
    UIBarButtonItem * leftItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"title_bar_back"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStyleDone target:self action:@selector(popViewController:)];
    self.navigationItem.leftBarButtonItem =leftItem;
    
    if (isGroup) {
        UIBarButtonItem * rigthItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"title_bar_more"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStyleDone target:self action:@selector(navRightBarItemTap:)];
    ...
    self.amplitudeImageView.hidden = YES;
...
- (void)showMenuViewController:(UIView *)showInView messageType:(MessageBodyType)messageType
{
    ...
    if (_atMenuItem == nil) {
         _atMenuItem = [[UIMenuItem alloc] initWithTitle:@"@xxx" action:@selector(atMenuAction:)];
        
        
    }
    
    if (messageType == MessageBodyType_Text) {
        [_menuController setMenuItems:@[_copyMenuItem, _deleteMenuItem,_atMenuItem]];
    }
    else{
        [_menuController setMenuItems:@[_deleteMenuItem,_atMenuItem]];
    }
...
-(MedieMessageType)getMessageMediaType:(NSString*)displayName{
    ...
    else if ([displayName hasSuffix:@".jpg"] || [displayName hasSuffix:@".png"] || [displayName hasSuffix:@".JPG"]|| [displayName hasSuffix:@".JPEG"]|| [displayName hasSuffix:@".jpeg"])
    {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *loginId = [userInfo valueForKey:@"loginId"];
    
    if (message.isGroup) {
        
        if (userName.length>0) {
            _inputTextView.text = [NSString stringWithFormat:@"@%@",userName];
        }
    }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.portraitImg.onTouchTapBlock = ^(UIImageView *imageView){
        NSString *jsonStr =     message.userData;
        SBJsonParser *jsonParser = [[SBJsonParser alloc] init];
        NSDictionary *userInfo = [jsonParser objectWithString:jsonStr];
        NSString *userName  = [userInfo valueForKey:@"name"];
...
#else
    NSArray *imagesArr = @[@"dialogue_image_icon",@"dialogue_camera_icon"];
    NSArray *textArr = @[@"图片",@"照相"];
    NSArray *selectorArr = @[@"pictureBtnTap:",@"cameraBtnTap:"];
#endif
...
-(void)recordButtonTouchDown{
    ...
    self.amplitudeImageView.hidden = NO;
...
-(void)saveMessageToLocal:(ECMessage*)message{
   
    ECTextMessageBody *body = (ECTextMessageBody*)message.messageBody;
    NSString *chatMessage = [NSString stringWithFormat:@"{‘from’:'%@','to':'%@','messageId':'%@','messageBody':'%@','timestamp':'%@','userData':'%@'}",message.from,message.to,message.messageId,body.text,message.timestamp,message.userData];
    [GTLModuleDataHelper saveUserChatInfo:chatMessage];
}
