...
-(instancetype) initWithIsSender:(BOOL)aIsSender reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier]) {
        ...
        if (self.isSender) {
            self.portraitImg = [[UIImageView alloc] initWithFrame:CGRectMake(DEVICE_WIDTH-50.0f, DefaultFrameY, 40.0f, 40.0f)];
            self.portraitImg.contentMode = UIViewContentModeScaleAspectFit;
            [self.contentView addSubview:self.portraitImg];
            

            NSString *iconUrlStr = [self getIconUrl];//[NSString stringWithFormat:@"%@%@",TL_SERVER_BASE_URL,[DemoGlobalClass sharedInstance].loginInfoDic[imageKey]];
            NSLog(@"iconURL:%@",iconUrlStr);
            [self.portraitImg sd_setImageWithURL:[NSURL URLWithString:iconUrlStr] placeholderImage:[UIImage imageNamed:@"ico_loading_logo"]];//.image =  [UIImage imageNamed:[DemoGlobalClass sharedInstance].loginInfoDic[imageKey]];
            
            self.bubbleView = [[UIView alloc] initWithFrame:CGRectMake(DEVICE_WIDTH-self.portraitImg.frame.origin.x-40.0f-10.0f, self.portraitImg.frame.origin.y+20, 40.0f, self.portraitImg.frame.size.height)];
            ...
            self.bubbleView = [[UIView alloc] initWithFrame:CGRectMake(self.portraitImg.frame.origin.x+self.portraitImg.frame.size.width+10.0f, self.portraitImg.frame.origin.y+20, 40.0f, self.portraitImg.frame.size.height)];
            UIImageView *bubleimg = [[UIImageView alloc] initWithFrame:self.bubbleView.bounds];
        ...
        _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, DEVICE_WIDTH, 30.0f)];
...
-(void)updateMessageSendStatus:(ECMessageState)state{
    ...
    bubbleFrame.origin.y = imageFrame.origin.y+20;
    ...
    NSString *iconUrlStr = [self getIconUrl];
    [self.portraitImg  sd_setImageWithURL:[NSURL URLWithString:iconUrlStr] placeholderImage:[UIImage imageNamed:@"ico_loading_logo"]];//.image = [[DemoGlobalClass sharedInstance] getOtherImageWithVoip:self.displayMessage.from];
}

-(NSString *)getIconUrl{
    

    NSString *jsonStr = self.displayMessage.userData;
...
-(NSString *)getMessageTime{
    ...
    //时间显示内容
        ...
        NSTimeInterval tempMilli = self.displayMessage.timestamp.longLongValue;
