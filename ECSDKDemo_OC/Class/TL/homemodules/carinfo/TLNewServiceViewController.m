...
@interface TLNewServiceViewController ()<PhotosViewDelegate>{
...
@property (nonatomic,strong) CMultiSelecter *serverTypeSelect;
@property (nonatomic,strong) NSArray *serverTypeArray;
@property (nonatomic,strong) TLCarServiceResult *detailDto;
@end

@implementation TLNewServiceViewController
...
-(void)updateUI{
    ...
    [rentTypeBtn setTitle:self.detailDto.serviceType forState:UIControlStateNormal];
    contentLabel.text = self.detailDto.serviceDesc;
...
-(void)publishBtnHandler{
    ...
    /*
     ...
     @property (nonatomic,copy) NSArray<TLImageDTO> *serviceImage;
     */
    
    TLSaveCarServiceRequest *request = [[TLSaveCarServiceRequest alloc] init];
    request.title = titleField.text;
    request.serviceType = serverType;
    request.address = cityId;
    request.serviceDesc = contentLabel.text;
    request.isTop = isTop;
    ...
    if (self.operateType.integerValue==1&&request.serviceImage.count==0) {
    ...
    if (request.title.length<=0||request.address.length<=0||request.serviceType.length<=0||request.serviceDesc<=0) {
        [GHUDAlertUtils toggleMessage:@"请填写服务相关信息"];
        return;
    }
...
-(void)addAllUIResources{
    ...
    activityUserLabel.text = @"服务类型：";
    activityUserLabel.font = FONT_16;
    activityUserLabel.textColor = COLOR_MAIN_TEXT;
    [rentTypeView addSubview:activityUserLabel];
    
    rentTypeBtn = [[RTextIconBtn alloc] initWithFrame:CGRectMake(hGap*2+100.f,  (textHeight-size16.height)/2, self.view.width-activityUserLabel.width-hGap*3, size16.height)];
    ...
    [rentTypeBtn setTitle:@"车辆维修" forState:UIControlStateNormal];
...
-(void)addRentTypeSelect{
    if (self.serverTypeSelect) {
        
        //        [UIView animateWithDuration:0.5f animations:^{
        //            [self.dateChooserView setAlpha:0];
        //        } completion:^(BOOL finished) {
        [self.serverTypeSelect removeFromSuperview];
        //        }];
    }
    __block     TLPersonCountDTO *first = self.serverTypeArray[0];
    NSMutableArray *dataArray = [[NSMutableArray alloc] init];
    [self.serverTypeArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        TLPersonCountDTO *dto = obj;
        NSDictionary *item = @{@"id":dto.codeValue,@"name":dto.codeName};
        if ([self.detailDto.serviceType isEqualToString:dto.codeName]) {
            first = dto;
        }
    ...
    self.serverTypeSelect = [[CMultiSelecter alloc] initWithFrame:CGRectMake(0.0f, CGRectGetHeight(self.view.frame), CGRectGetWidth(self.view.frame), 250.0f) itemData:@{@"id":first.codeValue,@"type":@"select",@"selectedId":first.codeValue,@"selectedName":first.codeName,@"data":dataArray,@"paramKey":@"personNum"}];
    WEAK_SELF(self);
    self.serverTypeSelect.OkBlock = ^(NSDictionary*data){
        [weakSelf selectChangeHandler:data];
    };
    [self.view addSubview:self.serverTypeSelect];
    //[self.serverTypeSelect setSelectedData:@{@"SELECT_INDEX":@"0"}];
...
-(void)selectChangeHandler:(NSDictionary*)data{
    ...
    [rentTypeBtn setTitle:str forState:UIControlStateSelected];
    //[[RAPPLICATION currentUser] setAccDay:[RUtiles stringFromDateWithFormat:[RUtiles dateFromString:dateStr format:@"yyyy-MM-dd"] format:@"yyyyMMdd"]];
    //[self reloadViews];
    serverType = [data valueForKey:@"id"];
...
-(void)selectBtnHandler:(UIButton*)btn{
    ...
    [self.serverTypeSelect showContentView];
}
