...
#import "TLHelper.h"
@implementation TLOrgMessageCell

-(void)initContent{
    
    TLOrgMessageDTO *cellDto = self.cellData;
    ...
    NSString *userIconUrl = [NSString stringWithFormat:@"%@%@",TL_SERVER_BASE_URL,cellDto.createUser.userIcon];
    if (userIconUrl) {
        ...
        userImageView.onTouchTapBlock = ^(UIImageView *imageView){
            [RTLHelper gotoUserInfoView:cellDto.createUser.loginId];
        };
    ...
    NSString *userName = cellDto.createUser.userName;
    ...
    //add commenttime str
    NSString *commentTime = cellDto.createDate;
    ...
    NSString *commentContextStr = cellDto.content;
