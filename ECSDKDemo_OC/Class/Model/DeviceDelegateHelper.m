 ...
 *  播放声音
 *
 *  @param messageType 1-all message  2-group message
 */
-(void)playRecMsgSound:(int)messageType{
    
    BOOL isPlaySound = YES;
   
    if (messageType==2) {
        isPlaySound = [GUserDefault valueForKey:IS_GROUP_MESSAGE_NOTICE_KEY]==nil||[[GUserDefault valueForKey:IS_GROUP_MESSAGE_NOTICE_KEY] isEqualToString:@"1"]?YES:NO;
    } else if(messageType==1){
        isPlaySound = [GUserDefault valueForKey:IS_ALL_MESSAGE_NOTICE_KEY]==nil||[[GUserDefault valueForKey:IS_ALL_MESSAGE_NOTICE_KEY] isEqualToString:@"1"]?YES:NO;
    }
...
#warning 时间全部转换成本地时间
    ...
    if (message.isGroup) {
        [self playRecMsgSound:2];
    }else{
        [self playRecMsgSound:1];
    }
